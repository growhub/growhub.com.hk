...
## 📮 お問い合わせフォーム（Cloudflare Pages Function + Email Routing）
...
### メール送信（Email Routing + `send_email` バインディング）

Pages Function は受信内容を、**Cloudflare Email Routing の検証済み宛先**（例: Gmail）へ
`send_email` バインディング経由でメール送信します。Cloudflare 側の設定手順:

1. **Email Routing を有効化**（対象ゾーンを Cloudflare に載せ、MX を設定）
2. **Destination Addresses** に受信用メール（例: あなたの Gmail）を追加し、確認メールで **検証**
3. 必要なら受信ルート（例: `contact@growhub.com.hk` → Gmail）も作成
4. Pages プロジェクトの **Settings → Functions → Bindings** で
   **Send email** バインディング `SEND_EMAIL` を追加し、`destination_address` に検証済み宛先を指定
5. 環境変数を設定（下表）
...
| 変数 / バインディング | 説明 |
| --- | --- |
| `SEND_EMAIL`（バインディング） | 種類 "Send email"。`destination_address` = 検証済み宛先 |
| `CONTACT_FROM` | 送信元（自ドメインのアドレス。例: `contact@growhub.com.hk`） |
| `CONTACT_TO` | 送信先（`SEND_EMAIL` の `destination_address` と一致させる。例: あなたの Gmail） |
| `TURNSTILE_SECRET_KEY` | Turnstile のシークレットキー（設定時のみサーバー検証を実施） |

送信メールは `Reply-To` に問い合わせ者のメールを設定するため、Gmail から**そのまま返信**できます。
...
2. お問い合わせフォームは **Cloudflare Pages Function（`functions/api/contact.ts`）で実装済み**。
   上記「お問い合わせフォーム」節のとおり、Email Routing の検証済み宛先 + `SEND_EMAIL`
   バインディング + `CONTACT_FROM` / `CONTACT_TO` / `TURNSTILE_SECRET_KEY` を設定すれば動作します。
