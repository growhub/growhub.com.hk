...
します（ローカル開発ではキー無しでOK）。チャレンジ用スクリプトが読み込めない環境では
送信をブロックしないフェイルオープンにしています。

> ⚠️ Netlify Forms 単体ではサーバー側でトークンを検証できないため、これは**フロント側の
> 抑止**にとどまります。厳密な検証を行う場合は、下記の Cloudflare Pages Functions などの
> サーバー処理で [Siteverify API](https://developers.cloudflare.com/turnstile/get-started/server-side-validation/)
> を用いて `cf-turnstile-response` を検証してください（`TURNSTILE_SECRET_KEY` が必要）。
...
2. お問い合わせフォームは Netlify Forms が使えなくなるため、
   ...
   Pages Functions を使う場合は、そのハンドラ内で Turnstile の `cf-turnstile-response` を
   [Siteverify API](https://developers.cloudflare.com/turnstile/get-started/server-side-validation/)
   で検証すると、サーバー側でもボットを弾けます（`TURNSTILE_SECRET_KEY` を環境変数に設定）
